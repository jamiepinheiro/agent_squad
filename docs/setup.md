...
See the **[WhatsApp guide](whatsapp.md)** for the complete setup, Muse encryption and key recovery, troubleshooting table, and contributor notes.
...
Conversation labels use WhatsApp history display names and phone-number mappings. Unnamed conversations show their distinct WhatsApp IDs, and invalid system addresses are excluded. WhatsApp AI conversations (`@bot`, including Muse) use a dedicated text transport that preserves their bot addresses. Muse prompts are wrapped in an authenticated encrypted envelope using the existing chat pairing key; own-device copies remain readable. The connector decrypts bot replies, reads their rich-text responses, and combines streaming edits into a single final reply. Bot session secrets are stored privately alongside the linked-device state and cleared when you unlink WhatsApp. Existing AI chats may require their original message-encryption key from phone history. When one is missing, Agent Squad requests the original message from the primary device and can also recover its key from that exact outgoing message in WhatsApp for Mac. This local lookup is read-only and recovered keys are cached only after authenticating a bot reply. Keep the chat available in WhatsApp for Mac if phone history omits its key; history replay is not guaranteed. The Baileys compatibility patch in `libraries/surfaces/whatsapp/scripts/patch-baileys.mjs` preserves bot destinations and session keys in own-device messages; installation and builds apply it and fail if the dependency changes incompatibly.
